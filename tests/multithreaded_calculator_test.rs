...
fn calculate_summa(a: Vec<Vec<u32>>, b: Vec<Vec<u32>>) -> Vec<Vec<u32>> {
...
fn identity() {
    ...
    assert_eq!(calculate_summa(a.clone(), identity.clone()), a);
    assert_eq!(calculate_summa(identity.clone(), a.clone()), a);
}
...
fn zero() {
    ...
    assert_eq!(calculate_summa(a.clone(), zero.clone()), zero);
    assert_eq!(calculate_summa(zero.clone(), a.clone()), zero);
}
...
fn example() {
    ...
    assert_eq!(
        calculate_summa(a.clone(), b.clone()),
